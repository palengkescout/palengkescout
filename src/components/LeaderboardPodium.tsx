          ...
          >
            <div className="relative">
              {rank === 1 && (
                <>
                  <span
                    className="absolute inset-0 rounded-full bg-palengke-gold/40 blur-md animate-podium-glow motion-reduce:animate-none"
                    aria-hidden="true"
                  />
                  <Crown
                    size={18}
                    className="absolute -top-5 left-1/2 -translate-x-1/2 text-palengke-gold-dark animate-crown-bounce motion-reduce:animate-none"
                    strokeWidth={2.4}
                  />
                </>
              )}
              <div className={`relative rounded-full p-0.5 ${rank === 1 ? "ring-2" : "ring-1"} ${medal.ring}`}>
                <Avatar name={entry.displayName} size={rank === 1 ? 56 : 44} />
              </div>
              <span
                className={`absolute -bottom-1 -right-1 w-5 h-5 rounded-full flex items-center justify-center text-[10px] font-bold ${medal.bg} ${medal.text}`}
              >
                {rank}
              </span>
            </div>
